...
- [ ] 启动前端测试台改造：
  ...
  - [ ] 右上角后续预留全局操作区：`dark mode`、通知铃铛、profile 入口。
...
- [ ] 全局头部和账户入口：
  - [ ] 右上角增加全局工具区，参考 `Tabler` 的 topbar 交互风格。
  - [ ] 增加 `dark mode` 开关，并把主题选择保存到本地。
  - [ ] 增加通知铃铛入口，首版可以先做未读状态和占位面板。
  - [ ] 增加 profile 入口卡片 / 下拉菜单，作为后续设置和登出入口。
- [ ] `Overview` 首页：
