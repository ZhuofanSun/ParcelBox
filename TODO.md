...
- [ ] 优先做 Phase 5 的事件持久化：
  - [ ] 把 door event、button request、snapshot、email send result 统一落库
  - [ ] 给 snapshot 建稳定的事件关联，而不只是在内存 payload 里携带
  - [ ] 把邮件账号密码改成环境变量读取，别继续直接写在 `config.py`
