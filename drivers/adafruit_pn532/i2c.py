...
class PN532_I2C(PN532):
    ...
    def __init__(
        self,
        i2c: I2C,  # i2c总线对象(board.I2C)
        address: int = _I2C_ADDRESS,  # 地址默认0x24 (i2cdetect -y 1)
        *,
        irq: Optional[DigitalInOut] = None,  # irq输出，中断引脚（optional）
        reset: Optional[DigitalInOut] = None,  # 硬件复位引脚（optional）
        req: Optional[DigitalInOut] = None,  # req引脚（optional）（32（H_Request），I2C 唤醒专用）
        debug: bool = False,  # True 会打印调试信息
    ) -> None:
    ...
    def _wakeup(self) -> None:
        ...
        if self._req:
            ...
            time.sleep(0.01)
        self.low_power = False  # 标记为非低功耗模式
        # 只有这里是必须的，配置 SAM，让 PN532 进入 NFC 工作模式
        self.SAM_configuration()  # Put the PN532 back in normal mode

    def _wait_ready(self, timeout: float = 1) -> bool:
        """
        Poll PN532 if status byte is ready, up to `timeout` seconds
        PN532在 i2c总线第一字节永远是 状态字节（status byte），需先用这个函数轮询等待
        0x00表示忙，0x01表示准备好，可以进行读写操作。
        """
        status = bytearray(1)  # 准备一字节缓存
        timestamp = time.monotonic()  # 当前时间戳，monotonic() 不受系统时间修改影响
        while (time.monotonic() - timestamp) < timeout:
            try:
                with self._i2c:
                    self._i2c.readinto(status)  # 尝试读取状态字节到 status变量
            except OSError:  # 忽略总线错误继续尝试
                continue
            if status == b"\x01":  # 01h 表示准备好
    ...
    def _read_data(self, count: int) -> bytearray:
        ...
        从 PN532 读取 count 字节的数据帧，不包括状态字节
        """
        # Build a read request frame.
        frame = bytearray(count + 1)  # +1 for status byte
        with self._i2c as i2c:
            i2c.readinto(frame, end=1)  # read status byte!
            if frame[0] != 0x01:  # not ready 要先用 wait_ready() 等待准备好
                raise BusyError
